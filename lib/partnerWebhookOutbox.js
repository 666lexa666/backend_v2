...
async function enqueuePartnerWebhook({ paymentId, notificationType, body = {} }) {
  ...
  const { data: refunds, error: refundsError } = await db
    ...
    .select('id,amount_minor,amount_currency_minor,status,provider_ref_id,provider_trx_id,requested_at,completed_at,updated_at,metadata')
    .eq('payment_pk', payment.payment_pk)
  ...
  const normalizedPayment = {
    ...
    account_currency: payment.account_currency || 'RUB',
    amount_currency_minor: payment.amount_currency_minor ?? payment.amount_minor ?? null,
    effective_currency_rate_rub: payment.effective_currency_rate_rub_snapshot ?? payment.currency_rate_rub_snapshot ?? null,
