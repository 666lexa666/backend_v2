...
async function executeCardPayment({ payment, runtime, input }) {
  ...
  const renderPublicUrl=process.env.RENDER_EXTERNAL_HOSTNAME ? `https://${process.env.RENDER_EXTERNAL_HOSTNAME}` : '';\n  const callbackBase=String(process.env.PUBLIC_API_URL || process.env.API_PUBLIC_URL || renderPublicUrl).replace(/\/$/,'');
