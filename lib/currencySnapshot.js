...
async function buildPaymentCurrencySnapshot({
  ...
  if (normalizedCurrency === 'RUB') {
    return {
      ...
      markupPercent: 0,
      effectiveRateRub: 1,
      amountCurrencyMinor: rubAmountMinor,
  ...
  const effectiveRateRub = officialRateRub * (1 + normalizedMarkup / 100);
  ...
  return {
    ...
    officialRateRub: roundRate(officialRateRub),
    markupPercent: roundRate(normalizedMarkup),
    effectiveRateRub: roundRate(effectiveRateRub),
    amountCurrencyMinor: Math.round(rubAmountMinor / effectiveRateRub),
    rateFetchedAt: rate.effective_at || new Date().toISOString(),
