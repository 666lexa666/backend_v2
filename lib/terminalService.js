...
async function getAdminCatalog(id) {
  const db = getSupabaseAdminClient();
  const { data, error } = await db
    .from('admin_terminal_catalog_v2')
    .select('*')
    .eq('id', id)
    .maybeSingle();
  if (error) throw error;
  if (!data) throw Object.assign(new Error('Терминал не найден'), { statusCode: 404, code: 'TERMINAL_NOT_FOUND' });
  return mapCatalog(data);
}

async function listCatalogAssignments(terminalId) {
  const db = getSupabaseAdminClient();
  ...
  const partnerIds = [...new Set(rows.map((row) => Number(row.partner_id)).filter(Boolean))];
  const projectIds = [...new Set(rows.map((row) => row.project_id).filter(Boolean))];

  const [{ data: partners, error: partnersError }, { data: projects, error: projectsError }] = await Promise.all([
    partnerIds.length
      ? db.from('partners').select('id,company_name,email,login,is_active,archived_at').in('id', partnerIds)
      : Promise.resolve({ data: [], error: null }),
    projectIds.length
      ? db.from('partner_projects').select('id,partner_id,name,is_active,archived_at').in('id', projectIds)
      : Promise.resolve({ data: [], error: null }),
  ]);
  ...
  return rows.map((row) => {
    const partner = partnerMap.get(Number(row.partner_id)) || null;
    const project = row.project_id ? projectMap.get(String(row.project_id)) || null : null;
    return {
      ...mapAssignment(row),
      partner_name: partner?.company_name || partner?.email || partner?.login || null,
      project_name: project?.name || null,
      partner: partner ? {
        id: partner.id,
        is_active: partner.is_active !== false,
        is_archived: Boolean(partner.archived_at),
      } : null,
      project: project ? {
        id: project.id,
        is_active: project.is_active !== false && !project.archived_at,
        is_archived: Boolean(project.archived_at),
      } : null,
    };
  });
}

async function setCatalogAssignmentActive(terminalId, assignmentId, isActive) {
  const db = getSupabaseAdminClient();
  const { data: current, error: findError } = await db
    .from('partner_terminals')
    .select('id,terminal_id,archived_at')
    .eq('id', assignmentId)
    .eq('terminal_id', terminalId)
    .maybeSingle();
  if (findError) throw findError;
  if (!current) throw Object.assign(new Error('Назначение терминала не найдено'), { statusCode: 404, code: 'ASSIGNMENT_NOT_FOUND' });
  if (isActive && current.archived_at) {
    throw Object.assign(new Error('Сначала восстановите назначение из архива'), { statusCode: 409, code: 'ASSIGNMENT_ARCHIVED' });
  }
  ...
  if (!isActive) update.is_default = false;
  ...
  return assignments.find((row) => String(row.id) === String(assignmentId)) || null;
}

async function getTerminalMonthlyTotals() {
  const db = getSupabaseAdminClient();
  const { data, error } = await db.rpc('get_terminal_monthly_totals_v2');
  if (error) throw error;
  return (data || []).map((row) => ({
    catalog_terminal_id: row.catalog_terminal_id,
    successful_count: Number(row.successful_count || 0),
    successful_amount: Number(row.successful_amount_minor || 0),
  }));
}

async function disablePartnerTerminal(partnerId, assignmentId) {
  const db = getSupabaseAdminClient();
  const { data, error } = await db
    .from('partner_terminals')
    .update({
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) throw Object.assign(new Error('Терминал партнёра не найден'), {
    statusCode: 404,
    code: 'PARTNER_TERMINAL_NOT_FOUND',
  });
  return data;
}
