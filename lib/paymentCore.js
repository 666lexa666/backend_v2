...
async function selectTerminalForPayment({ partnerId, projectId = null, terminalId = null, method = 'SBP', amountMinor }) {
  ...
  const rows = data || [];
  if (terminalId) {
    return rows.find((row) => row.id === terminalId || row.terminal_id === terminalId) || null;
  }
...
async function loadTerminalRuntime(assignment) {
  ...
  const { data: link, error: linkError } = await db
    ...
    .select('id,partner_id,project_id,terminal_id,label,provider_config,settings,min_amount_minor,max_amount_minor,is_active')
    .eq('id', assignment.id)
    .maybeSingle();
...
async function createPaymentCore(input) {
  ...
  const { data: payment, error } = await db.from('payments').insert({
    ...
    currency_rate_rub_snapshot: input.currencyRateRub ?? 1,
    metadata,
  }).select('*').single();
  if (error) throw error;
