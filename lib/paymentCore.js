...
async function createPaymentCore(input) {
  ...
  const transactionCurrency=String(input.transactionCurrency || 'RUB').trim().toUpperCase();
  if(transactionCurrency!=='RUB') throw apiError(400,'UNSUPPORTED_CURRENCY','Сейчас поддерживаются платежи только в RUB');
  ...
  const currencySnapshot=await buildPaymentCurrencySnapshot({
    ...
    markupPercent:input.currencyMarkupPercent,
  });

  const assignment=input.preselectedAssignment || await selectTerminalForPayment({
    partnerId,
  ...
  const runtime=input.preselectedRuntime || await loadTerminalRuntime(assignment);
  ...
  const {data:payment,error}=await db.rpc('create_payment_v2',{
    ...
    p_currency_rate_rub:currencySnapshot.officialRateRub,
    ...
    p_metadata:{
      ...metadata,
      currencyRateSource:currencySnapshot.rateSource,
    },
  ...
  return {payment,runtime,requestId:payment.request_id,currencySnapshot};
}
